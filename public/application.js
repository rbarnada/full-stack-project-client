...
var apiUrl = void 0;
var apiUrls = {
  production: 'https://run-tracker-api.herokuapp.com',
...
var runEvents = __webpack_require__(11);

$(function () {
  authEvents.addHandlers();
  runEvents.addHandlers();
});
...
var signUpSuccess = function signUpSuccess(data) {
  ...
  setTimeout(function () {
    return $('#up-message').css('background-color', 'white');
  }, 3000);
  // console.log(data)
...
var signInFailure = function signInFailure(data) {
  ...
  setTimeout(function () {
    return $('#in-error-message').text('');
  }, 3000);
  setTimeout(function () {
    return $('#in-error-message').css('background-color', 'white');
  }, 3000);
};
...
var changePassFailure = function changePassFailure(data) {
  ...
  setTimeout(function () {
    return $('#pass-error-message').text('');
  }, 3000);
  setTimeout(function () {
    return $('#pass-error-message').css('background-color', 'white');
  }, 3000);
};
...
var signOutSuccess = function signOutSuccess(data) {
  ...
  setTimeout(function () {
    return $('#status-message').css('border', 'none');
  }, 3000);
  $('body').addClass('home');
  store.user = null;
...
var onIndexRuns = function onIndexRuns(event) {
  event.preventDefault();
  $('#run-display').text('');
  $('#stat-display').text('');
  // console.log('button pressed')
  api.indexRuns().then(ui.indexRunsSuccess)
  // .then(ui.statsData)
...
var onDeleteRun = function onDeleteRun(event) {
  ...
  if (confirm('Are you sure you want to delete this run')) {
    api.deleteRun().then(ui.deleteRunSuccess).catch(ui.deleteRunFailure);
  }
};
...
var onUpdateRun = function onUpdateRun(event) {
  ...
  api.updateRun(data).then(ui.updateRunSuccess).catch(ui.updateRunFailure);
...
var indexRuns = function indexRuns() {
  // console.log('api connected')
  return $.ajax({
    url: config.apiUrl + '/runs',
    method: 'GET',
...
var deleteRun = function deleteRun() {
  // console.log('api connected')
  return $.ajax({
...
var updateRun = function updateRun(data) {
  // console.log('api connected')
  return $.ajax({
...
var addRunSuccess = function addRunSuccess(data) {
  // console.log('successful signup')
...
var addRunFailure = function addRunFailure(data) {
  // console.log('signup failure')
...
var indexRunsSuccess = function indexRunsSuccess(data) {
  // re-sorts runs so most recent is display first
  var sorted = data.runs.sort(function (a, b) {
    return b.id - a.id;
  });
  ...
  if (sorted.length === 0) {
    ...
    $('.update-button').on('click', function (event) {
      ...
      var runUpdate = $(event.target).attr('data-id');
  ...
  // ALL STATS DATA
  var statsData = function statsData(data) {
    // STATS PAGE INFO
    ...
    if (data.runs.length > 0) {
      ...
      var totalDistance = distanceArray.reduce(function (acc, val) {
        return acc + val;
      });
      ...
      var longest = distanceArray.reduce(function (acc, val) {
      ...
      var shortest = distanceArray.reduce(function (acc, val) {
      ...
      var addedTime = add(timeArray);

      // PREPENDS ZEROES IN SECONDS SPOT IF NUMBER IS BETWEEN 0-9
      // I.E. 1 -> 01
      var prependZero = function prependZero() {
        for (var i = 0; i < addedTime.length; i++) {
          if (addedTime[i] <= 9) {
            console.log('true');
      ...
      var addedTimeString = addedTime.toString();
      // console.log(addedTimeString)
      var normalizedTime = addedTimeString.replace(/,/g, ':');
...
var indexRunsFailure = function indexRunsFailure(data) {
  $('#status-message').text('An issue occured when getting runs');
  $('#status-message').css('background-color', '#F2DEDE');
};

var deleteRunSuccess = function deleteRunSuccess(data) {
...
var deleteRunFailure = function deleteRunFailure(data) {
  $('#status-message').text('An issue occurred when deleting runs');
  $('#status-message').css('background-color', '#F2DEDE');
};

var updateRunSuccess = function updateRunSuccess(data) {
...
var updateRunFailure = function updateRunFailure(data) {
  ...
  setTimeout(function () {
    return $('#update-error-message').text('');
  }, 3000);
  setTimeout(function () {
    return $('#update-error-message').css('background-color', 'white');
  }, 3000);
};
...
module.exports = {
  ...
  indexRunsSuccess: indexRunsSuccess,
  indexRunsFailure: indexRunsFailure,
  updateRunSuccess: updateRunSuccess,
...
const { splitDateTime, formatDateTime, DATETIME, DATETIME_LEN, TIME_SEPARATOR } = __webpack_require__(15)
...
module.exports = __webpack_require__.p + "efdffa8c7d42fe786d1386f2481e28e5.jpg";
